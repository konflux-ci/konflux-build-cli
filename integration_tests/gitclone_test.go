...
func newGitClone(params *gitclone.Params) (*gitclone.GitClone, error) {
	executor := cliwrappers.NewCliExecutor()
	gitCli, err := git.NewGitCli(executor, filepath.Join(params.OutputDir, params.Subdirectory))
	if err != nil {
		return nil, err
	}

	return &gitclone.GitClone{
		Params: params,
		CliWrappers: gitclone.CliWrappers{
			GitCli: gitCli,
		},
...
func Test_GitClone_SSLVerifyDisabled(t *testing.T) {
	...
	// After Run(), environment variables should be restored to their original state.
	// The clone succeeding with SSLVerify=false confirms that GIT_SSL_NO_VERIFY was

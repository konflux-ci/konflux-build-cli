...
const (
	...
	zotRegistryNetworkName   = "zot-network"
	...
	zotRegistryStorageCleanOnStart = true
...
func (z *ZotRegistry) GetRegistryDomain() string {
	return zotRegistryContainerName + ":" + zotRegistryPort
}
...
func (z *ZotRegistry) Start() error {
	...
	z.container.AddNetwork(zotRegistryNetworkName)
	...
	if zotRegistryStorageVolume {
		if zotRegistryStorageCleanOnStart {
			if err := os.RemoveAll(zotRegistryStorageHostDir); err != nil {
				// Failed to clear data in the volume dir, retry within the container.
				shouldCleanData = true
			}
		}
		z.container.AddVolume(zotRegistryStorageHostDir, zotDataPathInContainer)
	}
	...
	if shouldCleanData {
		if err := z.container.ExecuteAndWait("rm", "-rf", zotDataPathInContainer+"/*"); err != nil {
			return err
		}
	}
...
func (z *ZotRegistry) CheckTagExistance(imageName, tag string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("error making request to registry: %v", err)
	}
...
func (z *ZotRegistry) Prepare() error {
	...
	if err := z.ensureContainerToolNetwork(executor); err != nil {
		return err
	}
...
func (z *ZotRegistry) ensureContainerToolNetwork(executor *cliWrappers.CliExecutor) error {
	stdout, stderr, _, err := executor.Execute(containerTool,
		"network", "ls", "-q", "--filter", fmt.Sprintf("name=%s", zotRegistryNetworkName))
	if err != nil {
		z.logger.Errorf("failed to check zot registry network: %s\n%s", stdout, stderr)
		return err
	}
	if stdout == "" {
		// Network is missing, create it
		stdout, stderr, _, err := executor.Execute(containerTool, "network", "create", zotRegistryNetworkName)
		if err != nil {
			z.logger.Errorf("failed to create zot registry network: %s\n%s", stdout, stderr)
			return err
		}
	}
	return nil

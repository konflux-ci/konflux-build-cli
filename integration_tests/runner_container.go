...
func (c *TestRunnerContainer) ExecuteAndWait(command string, args ...string) error {
	c.ensureContainerRunning()
...
func (c *TestRunnerContainer) DebugCli(cliArgs ...string) error {
	c.ensureContainerRunning()
...
func (c *TestRunnerContainer) AddTaskResult(result string) {
	c.results[result] = path.Join(ResultsPathInContainer, result)
}

func (c *TestRunnerContainer) GetTaskResultValue(result string) (string, error) {

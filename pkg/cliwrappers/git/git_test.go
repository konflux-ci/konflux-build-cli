...
func Test_NewCli_versionCheck(t *testing.T) {
	...
	t.Run("should fail when git version is below minimum", func(t *testing.T) {
		...
		cli, err := NewGitCli(executor, "/tmp")
		g.Expect(err).To(HaveOccurred())
...
func newTestGitCli(execFunc func(workdir, command string, args ...string) (string, string, int, error)) *GitCli {
	return &GitCli{
		Executor: &mockExecutor{executeInDirFunc: execFunc},
...
func Test_FetchTags(t *testing.T) {
	...
	t.Run("should return error if fetch fails", func(t *testing.T) {
		cli := newTestGitCli(func(workdir, command string, args ...string) (string, string, int, error) {
			return "", "error", 0, errors.New("network error")
		})

		_, err := cli.FetchTags()

		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Error()).To(ContainSubstring("git fetch --tags failed"))
	})

	t.Run("should return error if fetch exits non-zero", func(t *testing.T) {
		cli := newTestGitCli(func(workdir, command string, args ...string) (string, string, int, error) {
			return "", "error", 1, nil
		})
		...
		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Error()).To(ContainSubstring("git fetch --tags failed with exit code 1"))
	})
}
...
func Test_FetchWithRefspec(t *testing.T) {
	...
	t.Run("should build correct args with all options", func(t *testing.T) {
		...
		err := cli.FetchWithRefspec(FetchOptions{
			Remote:      "origin",
	...
	t.Run("should reject empty remote", func(t *testing.T) {
		...
		err := cli.FetchWithRefspec(FetchOptions{Remote: ""})
...
func Test_SubmoduleUpdate(t *testing.T) {
	...
	t.Run("should return error on failure", func(t *testing.T) {
		...
		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Error()).To(ContainSubstring("git submodule update failed"))
	})
}

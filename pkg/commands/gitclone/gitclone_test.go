...
func Test_GitClone_performClone(t *testing.T) {
	...
	t.Run("should clone with basic parameters using init+fetch+checkout", func(t *testing.T) {
		...
		isFetchCalled := false
		_mockGitCli.FetchWithRefspecFunc = func(opts git.FetchOptions) error {
			isFetchCalled = true
...
func Test_GitClone_validateParams_subdirectory(t *testing.T) {
	...
	t.Run("should reject absolute subdirectory", func(t *testing.T) {
		c := &GitClone{
			Params: &Params{URL: "https://github.com/user/repo", Subdirectory: "/etc/passwd"},
		}
	...
	t.Run("should reject subdirectory escaping output dir", func(t *testing.T) {
		// A subdirectory that passes the .. check but still escapes via symlink or other means
		tmpDir := t.TempDir()
		c := &GitClone{
			Params: &Params{
				URL:          "https://github.com/user/repo",
...
func Test_saveAndRestoreEnv(t *testing.T) {
	g := NewWithT(t)

	t.Run("should restore previously set env var", func(t *testing.T) {
		key := "TEST_SAVE_RESTORE_SET"
		g.Expect(os.Setenv(key, "original")).To(Succeed())
		defer func() { _ = os.Unsetenv(key) }()

		restore := saveAndRestoreEnv(key)

		// Modify the env var
		g.Expect(os.Setenv(key, "modified")).To(Succeed())
		g.Expect(os.Getenv(key)).To(Equal("modified"))

		// Restore should bring back the original
		restore()
		g.Expect(os.Getenv(key)).To(Equal("original"))
	})

	t.Run("should unset env var that was not originally set", func(t *testing.T) {
		key := "TEST_SAVE_RESTORE_UNSET"
		_ = os.Unsetenv(key) // Ensure not set

		restore := saveAndRestoreEnv(key)

		// Set the env var
		g.Expect(os.Setenv(key, "new-value")).To(Succeed())
		g.Expect(os.Getenv(key)).To(Equal("new-value"))
	...
	t.Run("should handle multiple env vars", func(t *testing.T) {
		key1 := "TEST_SAVE_RESTORE_MULTI_1"
		key2 := "TEST_SAVE_RESTORE_MULTI_2"
		g.Expect(os.Setenv(key1, "val1")).To(Succeed())
		_ = os.Unsetenv(key2)
		defer func() {
			_ = os.Unsetenv(key1)
			_ = os.Unsetenv(key2)
		...
		restore := saveAndRestoreEnv(key1, key2)

		g.Expect(os.Setenv(key1, "changed")).To(Succeed())
		g.Expect(os.Setenv(key2, "added")).To(Succeed())

		restore()
		g.Expect(os.Getenv(key1)).To(Equal("val1"))
		_, exists := os.LookupEnv(key2)
		g.Expect(exists).To(BeFalse())
	})
}
...
func Test_GitClone_validateParams_depth(t *testing.T) {
	...
	t.Run("should fail with retry-max-attempts exceeding upper bound", func(t *testing.T) {
		c := &GitClone{
			Params: &Params{URL: "https://github.com/user/repo", RetryMaxAttempts: 101},
		}
	...
	t.Run("should pass with retry-max-attempts at upper bound", func(t *testing.T) {
		c := &GitClone{
			Params: &Params{URL: "https://github.com/user/repo", RetryMaxAttempts: 100},
		}

...
func (c *GitClone) mergeTargetBranch() error {
	...
	if c.Params.MergeSourceRepoURL != "" {
		...
		if normalizedOrigin == normalizedMerge {
			...
			if _, err := c.CliWrappers.GitCli.RemoteAdd(c.getCheckoutDir(), mergeRemote, c.Params.MergeSourceRepoURL); err != nil {
				return err
			}
		}
	}
	...
	currentSha, err := c.CliWrappers.GitCli.RevParse(c.getCheckoutDir(), "HEAD", false, 0)
	if err != nil {
	...
	merge, err := c.CliWrappers.GitCli.Merge(c.getCheckoutDir(), mergeRef, message)
	if err != nil {
		return err
	}
